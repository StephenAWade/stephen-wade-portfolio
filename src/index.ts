...
const validation = [
    ...
    check('lastName', 'A valid first name is required.').not().isEmpty().trim().escape(),
    check('email', 'Please provide a valid email.').isEmail(),
    // check('subject').optional().trim().escape(),
    check('emailMessage', 'A message of up to 2000 characters is required.').trim().escape().isLength({min: 1, max:2000})
]
...
const handlePostRequest = (request: Request, response: Response) => {
    ...
    const {firstName, lastName, email, emailMessage} = request.body
